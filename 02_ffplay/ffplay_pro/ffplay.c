...
typedef struct AudioParams {
    ...
    enum AVSampleFormat
        fmt; // 音频采样格式，比如AV_SAMPLE_FMT_S16表示为有符号16bit深度，交错排列模式。
...
typedef struct Decoder {
    ...
    int64_t
        next_pts; // 记录最近一次解码后的frame的pts，当解出来的部分帧没有有效的pts时则使用next_pts进行推算
...
typedef struct VideoState {
    ...
    double
        max_frame_duration; // 一帧最大间隔. above this, we consider the jump a timestamp discontinuity
...
static const char *wanted_stream_spec[AVMEDIA_TYPE_NB] = {0};   // 对应 ast
...
static void packet_queue_start(PacketQueue *q)
{
    SDL_LockMutex(q->mutex);
    q->abort_request = 0;
    packet_queue_put_private(q, &flush_pkt); //这里放入了一个flush_pkt，重置帧
    SDL_UnlockMutex(q->mutex);
}
        ...
        do {
            ...
            if (d->queue->serial != d->pkt_serial) {
                printf("%s(%d) discontinue:queue->serial:%d,pkt_serial:%d\n", __FUNCTION__,
                       __LINE__, d->queue->serial, d->pkt_serial);
            }
 ...
 * @param pic_sar
 */
static void calculate_display_rect(SDL_Rect *rect, int scr_xleft, int scr_ytop, int scr_width,
...
static int upload_texture(SDL_Texture **tex, AVFrame *frame, struct SwsContext **img_convert_ctx)
{
    ...
    if (realloc_texture(
            tex, sdl_pix_fmt == SDL_PIXELFORMAT_UNKNOWN ? SDL_PIXELFORMAT_ARGB8888 : sdl_pix_fmt,
...
static void set_sdl_yuv_conversion_mode(AVFrame *frame)
{
    ...
    if (frame
        && (frame->format == AV_PIX_FMT_YUV420P || frame->format == AV_PIX_FMT_YUYV422
            ...
            if (vp->pts >= sp->pts + ((float)sp->sub.start_display_time / 1000)) {
                if (!sp->uploaded) {
                    ...
                    if (realloc_texture(&is->sub_texture, SDL_PIXELFORMAT_ARGB8888, sp->width,
                                        sp->height, SDL_BLENDMODE_BLEND, 1)
                        < 0)
                        return;
...
static void update_volume(VideoState *is, int sign, double step)
{
    ...
    is->audio_volume =
        av_clip(is->audio_volume == new_volume ? (is->audio_volume + sign) : new_volume, 0,
                SDL_MIX_MAXVOLUME);
}
...
static void video_refresh(void *opaque, double *remaining_time)
{
    ...
    retry:
        if (frame_queue_nb_remaining(&is->pictq) == 0) { // 所有帧已显示
            ...
            if (frame_queue_nb_remaining(&is->pictq)
                > 1) { //只有有nextvp才会分析是否有必要检测是否该丢帧
                Frame *nextvp = frame_queue_peek_next(&is->pictq);
                duration = vp_duration(is, vp, nextvp);
                if (!is->step // 非逐帧模式才检测是否需要丢帧 is->step==1 为逐帧播放
                    && (framedrop > 0 || // cpu解帧过慢
                        (framedrop
                         && get_master_sync_type(is) != AV_SYNC_VIDEO_MASTER)) // 非视频同步方式
...
static int configure_audio_filters(VideoState *is, const char *afilters, int force_output_format)
{
    ...
    ret = avfilter_graph_create_filter(&filt_asrc, avfilter_get_by_name("abuffer"),
                                       "ffplay_abuffer", asrc_args, NULL, is->agraph);
    if (ret < 0)
        goto end;
...
static int video_thread(void *arg)
{
    ...
    for (;;) { // 循环取出视频解码的帧数据
        ...
        if (last_w != frame->width || last_h != frame->height || last_format != frame->format
            ...
            av_log(NULL, AV_LOG_DEBUG,
                   "Video frame changed from size:%dx%d format:%s serial:%d to size:%dx%d "
                   "format:%s serial:%d\n",
                   last_w, last_h,
            ...
            if ((ret = configure_video_filters(
                     graph, is, vfilters_list ? vfilters_list[is->vfilter_idx] : NULL, frame))
                < 0) {
                SDL_Event event;
...
static int audio_decode_frame(VideoState *is)
{
    ...
    if (af->frame->format != is->audio_src.fmt || dec_channel_layout != is->audio_src.channel_layout
        ...
        if (!is->swr_ctx || swr_init(is->swr_ctx) < 0) {
            av_log(NULL, AV_LOG_ERROR,
                   "Cannot create sample rate converter for conversion of %d Hz %s %d channels to "
                   "%d Hz %s %d channels!\n",
...
void show_help_default(const char *opt, const char *arg)
{
    ...
    printf("\nWhile playing:\n"
           ...
           "left/right          seek backward/forward 10 seconds or to custom interval if "
           "-seek_interval is set\n"
    ...
    if (!display_disable) {
        ...
        if (alwaysontop)
            ...
            av_log(NULL, AV_LOG_WARNING,
                   "Your SDL version doesn't support SDL_WINDOW_ALWAYS_ON_TOP. Feature will be "
                   "inactive.\n");
#endif
        if (borderless)
